...
v0.3.3 therefore accepts a known diagnostic uncertainty; it does not establish
...
- Record the accepted dependency exceptions: Torch 2.12.1 remains on the
  ...
  9.0.3 release.
- Confirm the dated `0.3.3` changelog entry matches the publication date.
...
- Confirm the release notes explain the v0.2 full-config compatibility fix,
  ...
  legacy-warning wording, the small nonzero probability display fix, and the
  composite/text architecture-fingerprint correction. Keep the v0.3.0 notes
  for the same-position migration, chunk persistence, diagnostics, hybrid-cache
  replay fix, and optional-kernel policy.
- Confirm no `v0.3.3` tag exists at another commit.
...
1. Either dispatch the `Release` workflow on `master`, or push the current
   ...
   `git push origin HEAD:refs/heads/release/v0.3.3`. A branch-triggered release
...
2. For a manual dispatch, enter `0.3.3` and acknowledge that the multi-GPU
...
it does not block v0.3.3. Use a fresh output directory, the exact published
v0.3.3 wheel or image, the same pinned model revision and prompt corpus for
...
`ghcr.io/returnmoe/miru-tracer:0.3.3-cu126`; the unqualified CUDA 13.0 image
