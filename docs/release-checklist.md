...
v0.3.0 therefore accepts a known diagnostic uncertainty; it does not establish
...
- Record the accepted dependency exceptions: Torch 2.12.1 remains on the
  ...
  9.0.3 release.
- Confirm the dated `0.3.0` changelog entry matches the publication date.
  Keep the Lucas Teske credit in the published section.
- Confirm the release section explains the same-position lens migration,
  existing fitted-lens compatibility, chunk persistence change, diagnostics,
  hybrid-cache replay fix, and optional-kernel policy.
- Confirm no `v0.3.0` tag exists at another commit.
...
it does not block v0.3.0. Use a fresh output directory, the exact published
v0.3.0 wheel or image, the same pinned model revision and prompt corpus for
...
`ghcr.io/returnmoe/miru-tracer:0.3.0-cu126`; the unqualified CUDA 13.0 image
